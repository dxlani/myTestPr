...
@VueComponent({
    template: require('./orderReleaseAdd.html'),
    style:require('./orderReleaseAdd.scss')
})

export class OrderReleaseAddComponent extends Vue {
    el:'#orderReleaseAdd'
    components:({
        vclienttable:any,
    })
    ...
    ready(){
        ...
        if(this.name == "copyInquiryRelease"){
             dataService().Inquiry.getInquiry(this.id).then((res)=>{
                this.addOrderTime = res.cspInquiryTime;
                ...
                dataService().CustomerRepresentative.getCustomerList("",0,-1).then((resConsignor)=>{
                    ...
                    this.consignorName = res.customerName;
                });
                ...
                this.searchGoodsName();
                this.searchSettleName();  //询价复制新增无结算单位
        ...
        }else if(this.name == "copy"){        //复制新增获取订单数据
            if(this.table=="1"){
                dataService().CspOrder.getCspOrder(this.id).then((res)=>{
                    ...
                    dataService().CspOrder.getAddOrderAuthAndSettleIsExis(this.settle).then((res)=>{
                        // if(res.addOrderAuth){
                        if(!res.settleIsExis){
                            bootbox.alert("结算单位名称不存在（"+ `${this.settle}` +"）");
                            this.settleId = "";
                            this.settle = "";
                        }
                        // }else{
    ...
    saveChildOrderLine = function(childValide){
        if(childValide){
            if(!this.timeJudge(this.startTime,this.arrivalTime)){
                return;
            }else if (this.settleId == "" || this.settleId == undefined) {
                bootbox.alert("请选择结算单位!");
                return;
    ...
    //发货发布
    addOrderSave = function(isValid){
        if(isValid){
            bootbox.confirm("确定发布此单？",(result)=>{
                if(result){
                    if(this.tableData.length == 0){
