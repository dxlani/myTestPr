...
@VueComponent({
    template: require('./inquiryAdd.html'),
    style: require('./inquiryAdd.scss')
})
export class InquiryAddComponent extends Vue {
    el: '#inquiryAdd'
    components: ({
        vclienttable: any,
        ossupload: any
     ...
     * 子询价单列表
     */
    columns = ['id', 'shipAddress', 'deliveryAddress', 'goodsName', 'goodsQuantity', 'plannedDeliveryTime', 'plannedArrivalTime', 'carType', 'carLength', 'operation']
    tableData = []
    inquiryChildList = [];
    options = {
        texts: {
        ...
        headings: {
            ...
            goodsName: "货物名称",
            goodsQuantity: "货物数量",
            plannedArrivalTime: "预计到货时间",
            plannedDeliveryTime: "预计发货时间",
            carType: "所需车型",
            carLength: "所需车长",
            operation: '操作'
        },
        templates: {
            operation: function (row) {
                return `<a class='glyphicon glyphicon-trash primary text-danger m-l-xs' title='删除' href='javascript:void(0);' @click='$parent.deleteMe(${row.index})'></a>`
            }
        },
    };
    sortable = [];
    deleteMe = function (index) {;
        var itemDel = this.tableData.filter(t => t.index == index)
        this.tableData.$remove(itemDel[0])
        var inquiryChildListItemDel = this.inquiryChildList.filter(t => t.index == index);
        this.inquiryChildList.$remove(inquiryChildListItemDel[0]);
    }
    ...
    pointB={};/* 终点坐标 */
    //设置禁用（详情）
    titleInquiry: string = '询价单详情'
    showLinkToInquiry: boolean = false;
    sasUrl: string = '';
    starProvinceList = [];
    starCityList = [];
    starAreaList = [];
    ...
    //提交的图片数据
    submitImageLists = [];
    uploadDisabled = this.imageLists.length > 0;
        ...
        templates: {
            operation: (row) => {
                ...
                if (row.showStatus) {
                    ...
                    return operationDownloadTemple
                }
            },
            image: (row) => {
                if (row.showStatus) {
    ...
    deleteImage = function (index) {
        ...
        if (this.imageLists.length <= 0) {
            $('#inquiryAdd_uploadAll').attr('disabled', 'true');
        }
    }
    //上传附件
    ...
    ready() {
        ...
        this.inquiryStatus = this.$route.query.status;
        ...
        if (this.name == "copy" || this.name == "add" || this.name == "copyInquiry") {
            this.titleInquiry = '询价单新增';
            this.inquiryTime="";
        } else if (this.name == "detail") {
            this.titleInquiry = '询价单详情';
            $('#fieldsetDisabled').attr('disabled', 'disabled');
            $("#fieldsetDisabledTwo").attr('disabled', 'disabled');
        }

        //时间插件 初始化
        $('#inquiryAdd_plannedArrivalTime').datetimepicker();
        $("#inquiryAdd_plannedDeliveryTime").datetimepicker();
        // 获取发布询价详情
        // if (this.inquiryId !== "undefined" && this.inquiryId && this.name !== 'add') {
        if (this.inquiryId !== "") {
            if(this.name == "copyInquiry"){
                dataService().Inquiry.getInquiry(this.inquiryId).then((res)=>{
                    //子询价编号
                    this.inquiryChildCode = res.inquiryChildId;
                    //询价发布时间
                    // this.inquiryTime = res.creationTime
                    //客户单位
            ...
            }else if(this.name == "copy" || this.name == "detail"){
     ...
     */
    editInquiry(){
        router.go('/app/inquiry/inquiryEdit/?id='+ this.inquiryId);
    }
    /**
     * 终结询价单
     */
    endInquiry(){
        bootbox.confirm("确认终结该询价单吗？",(bootboxResult)=>{
            if(bootboxResult){
                dataService().CspInquiry.editCspInquiry(this.inquiryId).then((res)=>{
                    if(res.success){
                        bootbox.alert("终结询价单成功",()=>{
                            router.go('/app/inquiry/inquiryReleaseManage')
                        })
                    }else{
                        if(res.errorCode=="2002"){
                            bootbox.alert("操作失败，该询价单已被受理",()=>{
                            })
                        }
                    }
                },(rej)=>{});
    ...
    addInquiryChild = function (childValid) {
        if (childValid) {
            ...
            if (startTime.getTime() >= endTime.getTime()) {
                ...
                this.tableData.push(inquiryChildItem);
            }
        }
    }
    /**
     * 询价发布
     */
    inquirySave = function (enableInquiryReleaseAdd) {
        if (enableInquiryReleaseAdd) {
            ...
            bootbox.confirm("确认发布询价吗？", (bootboxResult) => {
                if (bootboxResult) {
                    ...
                    dataService().CspInquiry.addCspInquiry(submitInquiry).then((res) => {
                        if (res.success) {
                            bootbox.alert("发布成功！",function(){
                                if(this.listName == "manageList"){
                                    router.go('/app/inquiry/inquiryManage')
                                }else if(this.listName == "confirmList"){
    ...
    onFileChange = function (e) {
        ...
        if (files.length > 0) {
            for (var i = 0; i < files.length; i++) {
                ...
                if (this.imageLists.length > 0) {
                    $('#inquiryAdd_uploadAll').removeAttr('disabled');
                }
            }
        }
        $("#upLoader").val("");
    ...
    handleFileSelect = function (e) {
        ...
        if (files.length > 0) {
            for (var i = 0; i < files.length; i++) {
                ...
                var imageName = Math.round(Math.random() * 10000).toString();
                // console.info("imageName",imageName);
                var objUrl = this.getObjectURL(files[i]);
                files[i].id = imageName;
                files[i].path = objUrl;
                this.imageLists.push(files[i]);
    ...
    getDistance=function(){
        this.startPoint=$('#inquiryAdd_popupProvince option:selected').text()+ $('#inquiryAdd_popupCity option:selected').text()+$('#inquiryAdd_popupCounty option:selected').text();
        this.endPoint=$('#inquiryAdd_receiveProvince option:selected').text()+ $('#inquiryAdd_receiveCity option:selected').text()+$('#inquiryAdd_receiveCounty option:selected').text();
