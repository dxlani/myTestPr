...
@VueComponent({
    ...
    style: require('./SumaccountDetail.scss')
})

export class SumaccountDetailComponent extends Vue{
    el:'#SumaccountDetail'
    components:({
        SumaccountDetail:any,
    })
    ...
    /* 下载电子回单合同 */
    downloadContract(){
        dataService().CheckFinance.downloadContract(this.contractNumber).then((res)=>{
    ...
    contractImgList = [];
     ...
     ready() {
        ...
        if(this.contractNumber != ""){
            dataService().CheckFinance.getContractDetail(this.contractNumber).then((res)=>{
        ...
        dataService().CheckFinance.GetOrderReceivableList(this.id).then((res) => {
            var receiveList =[];
