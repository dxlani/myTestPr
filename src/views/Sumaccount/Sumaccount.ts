...
@VueComponent({
    ...
    style:require('./Sumaccount.scss')
})

export class Sumaccount extends Vue {
    el: '#sumaccount'
    components: ({
        Sumaccount: any,  
    })
                ...
                { field: "contractNumber", title: "电子回单", sortable: false, },
                {
                    ...
                    events: {
                        'click .detailOrder': function (e, value, row, index) {
                            router.go('SumaccountDetail/?id=' + row.id + "&contractNumber=" + row.contractNumber );
                        },
                    },
                }
            ],
    ...
    load = function (skip, count) {
        dataService().CheckFinance.GetOrderFinceList(this.checkQuery.startTime, this.checkQuery.endTime, "", this.checkQuery.originAddress, this.checkQuery.destinationAddress,
    ...
    //获取列表总费用
    loadtotal = function () {
        dataService().CheckFinance.GetPriceTotle(this.checkQuery.startTime, this.checkQuery.endTime, "", this.checkQuery.originAddress, this.checkQuery.destinationAddress,"",this.checkQuery.orderNumber,this.checkQuery.receiptStatus).then((res) => {
            this.priceTotal = res;
        }, function (rej) {
        });
    ...
    downExport = function () {
        dataService().CheckFinance.GetOrderFinceExport(this.logisticsId,this.clientId,this.checkQuery.startTime, this.checkQuery.endTime, "", this.checkQuery.originAddress, this.checkQuery.destinationAddress,"",this.checkQuery.orderNumber,this.checkQuery.receiptStatus,0, this.records);
