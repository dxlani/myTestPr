...
@VueComponent({
    template: require('./orderManageDelivered.html'),
})

export class OrderManageDelivered extends Vue {
    el:'#orderManageDelivered'
    components:({
        vclienttable:any,
    })
            ...
            singleSelect:true,
                ...
                onCheck: (row, $element) =>{
                    ...
                    if(row.locationPhone == ""){
                        ...
                        this.locationPhone=row.locationPhone;
                    }
    ...
    localHistory = function(state){
        if(state){
            ...
            if(routerName.search("OrderManageDelivered")>0){
    ...
    location = function(){
        ...
        router.go("./location?phone="+this.locationPhone+"&carCode="+this.carCode+"&originAddress="+this.originAddress+"&destinationAddress="+this.destinationAddress + "&orderId=" + this.orderId)
    }
    /**
     * 硬件定位
     */
    ...
    getBDNPLocation = function(){
        router.go("/app/order/BDNPLocation?carCode="+this.carCode+"&originAddress="+this.originAddress+"&destinationAddress="+this.destinationAddress + "&orderId=" + this.orderId);
    }
