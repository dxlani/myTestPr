...
(function () {
	...
	Canvas.Img.prototype._setRandomProperties = function(oConfig) {
		...
		if (oConfig.top == null) {
			this.top = this.height / 2 + document.body.scrollHeight  / 2 - 131.5;
			console.log("document.body.scrollHeight/2",document.body.scrollHeight / 2);
			// this.top = this.height / 2 + Math.random() * 500;
		}
		
		if (oConfig.left == null) { 
			this.left = this.width / 2 + document.body.scrollWidth  / 2 -175;
			console.log("document.body.scrollWidth/2", document.body.scrollWidth / 2)
